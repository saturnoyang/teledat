...
switch($op){
	...
	case 1:
		...
		$consulta="SELECT h_retorno.id_retorno, h_retorno.id_instalador, h_retorno.id_usuario, h_retorno.fecha, ".
					"DATE_FORMAT(h_retorno.fecha,'%d-%m-%Y') as rt_fecha, h_retorno.comentario, h_retorno.status, ".
				...
				"FROM h_retorno, usuarios, instaladores ".
				"WHERE h_retorno.id_usuario = usuarios.id_usr ".
					"AND h_retorno.id_instalador = instaladores.id_inst AND h_retorno.status = '0'";
/*


SELECT h_retorno.id_retorno, h_retorno.id_instalador, h_retorno.id_usuario, h_retorno.fecha, DATE_FORMAT(h_retorno.fecha,'%d-%m-%Y') as rt_fecha, 
	h_retorno.comentario, h_retorno.status, usuarios.id_usr, usuarios.usr_nombre, 
...
FROM h_retorno, usuarios, instaladores WHERE h_retorno.id_usuario = usuarios.id_usr AND h_retorno.id_instalador = instaladores.id_inst AND 
		h_retorno.status = '0'
...
*/
		...
		if ($result = $mysqli->query($consulta)) {
			...
			if($numero_resultados==0){
				echo "<form id='form' name='form' method='post'  autocomplete='off' action='".basename(__FILE__)."?op=22&usr=$usr'>\n".
						...
						"<td>Instalador, que hace la devolución de la herramienta</td>\n".
						"<td align='center'><select name='status' id='status'>\n".
											"<option value='0'>Seleccione...</option>\n".
											"<option value='1'>Iniciar Devolución</option>\n".
											"<option value='2'>Cerrar Devolucion</option>\n".
						"</select></td>\n</tr>".
						"<tr>\n<td>\n";
		...
		if ($result2 = $mysqli->query($consulta)) {
			...
			echo "Buscar registo de Devolución de herramientas";
				...
				foreach($rows as $row){
					...
					echo "<Td colspan='3'>Devolución de Herramientas: ".$row['id_retorno']."</td></tr><tr>"; 
					echo "<td>Fecha de Prestamo</td><td>Instalador</td><td>Usuario</td></tr><tr>";
					echo "<Td>".$row['rt_fecha']."</td>";
					...
					echo "<td><button onclick=\"window.location.href='".basename(__FILE__)."?op=22&usr=".$row['id_usr'].
									"&id=".$row['id_retorno']."'\">Continuar</button></td>";
	...
	case 22:
		...
		if((empty($_POST['status'])? 0 :$_POST['status']) == 2 ){
			/*
			pasos: 	1) buscar las herramientas del pedido y marcarla como libre

					2) marcar el pedido como finalizad0.
			*/
			
		$consulta=	"SELECT id_retorno, id_herramienta FROM h_ret_dtalle WHERE id_retorno='$id'";
		$mysqli = new mysqli($db_host, $db_usuario, $db_passwd, $db_nombre);
		...
		if ($result = $mysqli->query($consulta)) {
			...
			if($numero_resultados==0){
				...
				foreach($rows as $row){
					...
					echo "<TR bgcolor=D0CFF9>\n";
					echo "<Td>".$row['id_retorno']."</td>";
					echo "<Td>".$row['id_herramienta']."</td>";

					echo "</tr>\n";
					// actualizar herramientas

					$consulta2 = "UPDATE herramientas SET id_instalador='0', id_prestamo='0' WHERE id_tool='".$row['id_herramienta']."'";
				...
				$rows2 = null;
					...
					$consulta2 = "UPDATE h_retorno SET status='1' WHERE id_retorno='".$id."'";
		...
		if($id==0){
			echo "Agregar: ";
			$consulta="INSERT INTO h_retorno (id_retorno, id_instalador, id_usuario, fecha, comentario, status) ".
			...
			echo "</td></tr>";
			$result = null;
			$consulta="SELECT id_retorno, id_instalador, id_usuario, fecha, comentario, status FROM h_retorno ".
			...
			$consulta="SELECT id_retorno, id_instalador, id_usuario, fecha, comentario, status FROM h_retorno WHERE id_retorno='$id' LIMIT 1";			
		}
			...
			if($numero_resultados==0){
				...
				foreach($rows as $row){
					...
					echo "<form id='form' name='form' method='post'  autocomplete='off' ".
							"action='".basename(__FILE__)."?op=22&inst=".$inst."&id=".$row['id_retorno']."&usr=$usr'>".							
							'<tr>'.
							...
							"<option value='0'>Seleccione...</option>\n".
							"<option value='1'>Iniciar Devolución</option>\n".
							"<option value='2' selected >Cerrar Devolución</option>".
							"</select></td></tr><tr><td>";
	...
	echo "</td>\n".
			...
			"</tr>\n</table>\n</form>\n";					
					...
					echo "<center><iframe id='pr_frame' name='pr_frame' align='middle' src='".basename(__FILE__)."?op=5&inst=".$inst."&id=".$row['id_retorno']."' ".
							"frameborder='1' style='overflow: hidden; height: 500; width: 80%; position: relative;' height='500' width='80%'>".
							"</iframe><br>\n".
						 "</center>";
	...
	case 5:
		...
		$consulta=	"SELECT h_ret_dtalle.id_retorno, h_ret_dtalle.id_herramienta , ".
					...
					"FROM herramientas, h_ret_dtalle ".
					"WHERE h_ret_dtalle.id_retorno='$id' ".
						"AND h_ret_dtalle.id_herramienta = herramientas.id_tool";
...
*/
	...
	case 51:
		...
		if( substr($_POST['busca'],0,3) == "[-]" ){
			$consulta="SELECT id_tool, id_instalador, id_prestamo, nombre, codigo1, codigo2, comentario, status  ".
						"FROM herramientas WHERE id_instalador='".$inst."' AND id_tool='".substr($_POST['busca'],3)."'";
		}else{
			...
			if($numero_resultados==0){
				...
				if($numero_resultados==1){
					...
					echo 	"<input name='herramienta' type='hidden' value='".$row['id_tool']."' />".
							"<input type='submit' name='button' id='button' value='Devolver'></td></tr></form>\n";
				}
				echo "</table>";
	...
	case 52:
		...
		$consulta="SELECT id_retorno, id_herramienta FROM h_ret_dtalle WHERE id_retorno='".$id."' AND id_herramienta ='".$_POST['herramienta']."' ";
		
		if ($result2 = $mysqli->query($consulta)) {
			$numero_resultados = $mysqli->affected_rows; 
			if($numero_resultados==0){ 
				$consulta="INSERT INTO h_ret_dtalle (id_retorno, id_herramienta) VALUES ('".$id."', '".$_POST['herramienta']."')";		
